...
export async function getProducts() {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/products`, { cache: 'no-store' })
    if (res.ok) {
      const dbProducts = await res.json()
      if (dbProducts && dbProducts.length > 0) {
        return dbProducts.map((p: any) => ({
          ...p,
          featuredImage: p.images ? { src: p.images[0], width: 1000, height: 1000, alt: p.title } : null
        }))
      }
    }
  } catch (e) {
  ...
  // Fallback to static data if API fails or is empty
  return [
    {
      id: 'gid://1001',
...
export async function getProductByHandle(handle: string) {
  ...
  handle = handle.toLowerCase()
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/api/products/${handle}`, { cache: 'no-store' })
    if (res.ok) {
    ...
    console.error('API Handle Error:', e)
  }
